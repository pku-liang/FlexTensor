...
def rnn_matexp():
    ...
    Whh = tvm.te.placeholder((num_hidden, num_hidden), name="Whh")
    s_init = tvm.te.compute((1, batch_size, num_hidden),
                         lambda _, i, j: 1.0, name="init")
    s_state = tvm.te.placeholder((num_step, batch_size, num_hidden))
    kh = tvm.te.reduce_axis((0, num_hidden), name="kh")
    s_update = tvm.te.compute(
        (num_step, batch_size, num_hidden),
        lambda t, i, j: tvm.te.sum(s_state[t-1, i, kh] * Whh[kh, j], axis=kh),
        name="update")
