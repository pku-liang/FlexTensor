...
@autotvm.template
def gemm_int8(n, m, l):
    A = tvm.te.placeholder((n, l), name='A', dtype='int8')
    B = tvm.te.placeholder((m, l), name='B', dtype='int8')

    k = tvm.te.reduce_axis((0, l), name='k')
    C = tvm.te.compute((n, m), lambda i, j: tvm.te.sum(A[i, k].astype('int32') * B[j, k].astype(
        'int32'), axis=k), name='C')
    ...
    s[C].bind(by, block_y)
    s[C].bind(bx, block_x)
    s[C].bind(tyz, tvm.te.thread_axis('vthread'))
    s[C].bind(txz, tvm.te.thread_axis('vthread'))
