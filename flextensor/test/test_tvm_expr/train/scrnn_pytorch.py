...
class SCRNCell(nn.Module):
    def __init__(self, input_size, num_units, context_units, alpha):
        ...
        self._contmext_units = context_units
        self._alpha = alpha
    ...
    def forward(self, inputs, state):
        # state_h.shape = (batch_size x num_units), state_c.shape = (batch_size x context_units)
        state_h, state_c = state.split([self._num_units, self._context_units], dim=1)
        ...
        hidden_state = torch.sigmoid(self.fc(torch.cat([context_state, inputs, state_h], dim=1)))
        # output.shape = (batch_size x num_units)
        output = hidden_state @ self.U + context_state @ self.V
        # new_state.shape = (batch_size x (num_units+context_units))
        new_state = torch.cat([hidden_state, context_state], dim=1)
