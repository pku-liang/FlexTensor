...
if __name__ == "__main__":
    ...
    print("%s baselines dilation convolution for target %s (%d):" % (args.type, args.target, args.device))
    for i, shape in enumerate(shapes):
        count = i + args.from_ 
        print("layer", count)
        batch, in_channel, H, W, out_channel, k, _, stride, padding, dilation, groups = shape
        cost = baseline(batch, H, W, in_channel, k, out_channel, stride=stride, padding=padding, dilation=dilation, groups=groups, number=args.number, dev=args.device)
        print("Use %f(ms)" % cost)
    print("Done!")
