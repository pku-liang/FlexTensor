...
def visit_for(node, ret):
    ...
    ext_type, extent = ret["stack"].pop()
    assert ext_type == "int32", "loop extent type must be int32"
    min_type, min_val = ret["stack"].pop()
    assert min_type == "int32", "loop min type must be int32"
    var_type, name = ret["stack"].pop()
    assert var_type == "int32", "loop var type must be int32"
    ret["loop_lst"].append((name, min_val, extent))
    visit(node.body, ret)
...
def visit_var(node, ret):
    ret["stack"].append((node.dtype, node.name))
...
def visit_float_imm(node, ret):
    ret["stack"].append((node.dtype, node.value))
