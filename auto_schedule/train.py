...
from auto_schedule.scheduler import graph_analysis, op_schedule_cpu_general_dx, parallel_evaluate, able_inline
...
def train_op_schedule_cpu_general_dx(entities, epoch, batch_size, path, loop_num=100, loop_size=32):
    ...
    for i in range(epoch):
        ...
        while count < batch_size:
            ...
            pre_cost = parallel_evaluate(s, bufs, "llvm", np.random.randint(0, MAX_CPU), 1, timeout=10.0)
            ...
            post_cost = parallel_evaluate(s, bufs, "llvm", np.random.randint(0, MAX_CPU), 10, timeout=10.0)
            ...
            for i in range(10):
                ...
                for op in bfs_order:
                    ...
                    if able_inline(op, down_graph):
                        ...
                        group_points.append(op)
                op_schedule_cpu_general_dx(dim, s, group_points[0], model, random=False, sampling=True)
                time_cost = parallel_evaluate(s, bufs, "llvm", np.random.randint(0, MAX_CPU), 5, timeout=10.0)
                if time_cost < best_time:
                    best_s = s
                    best_bufs = bufs
                    best_time = time_cost
            final_time = parallel_evaluate(best_s, best_bufs, "llvm", np.random.randint(0, MAX_CPU), 10, timeout=10.0)
            print("{}{} use {}ms\n".format(func.__name__, args, final_time))
