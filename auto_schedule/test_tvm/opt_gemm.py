...
def test(bx, by, fn):
   # k1 = tvm.reduce_axis((0, K // fn), 'k1')
   # k2 = tvm.reduce_axis((0, fn), 'k2')
   k = tvm.reduce_axis((0, K), 'k')
   A = tvm.placeholder((M, K), name='A')
   B = tvm.placeholder((K, N), name='B')
   # packedA = tvm.compute((M // bn, K // fn, bn, fn), lambda i, j, k, l: A[i * bn + k, j * fn + l], name='packedA')
   # packedB = tvm.compute((N // bn, K, bn), lambda x, y, z: B[y, x * bn + z], name='packedB')
   # C = tvm.compute((M, N),
   #                lambda x, y: tvm.sum(A[x, k] * packedB[y / bn, k, y % bn], axis=[k]),
   #                name = 'C')
   ...
   xo, yo, xi, yi = s[C].tile(s[C].op.axis[0], C.op.axis[1], bx, by)
   ko, ki = s[C].split(s[C].op.reduce_axis[0], factor=fn)
   s[C].reorder(xo, yo, ko, xi, ki, yi)
   ...
   # xc, yc = s[CC].op.axis
   #
   # ko, ki = s[CC].op.reduce_axis
   # ko, ki = s[CC].split(k, factor=fn)
   # s[CC].reorder(ko, xc, ki, yc)
   # s[CC].unroll(ki)
   # s[CC].vectorize(yc)
   ...
   # a, b, c, d = s[packedA].op.axis
   # s[packedA].parallel(a)
   # s[packedA].vectorize(d)
...
# best_mem = [1, 1, 1]
# worst_mem = [1, 1, 1]
# for bx in [4, 8, 16, 32, 64, 128, 256]:
#     for by in [4, 8, 16, 32, 64, 128, 256]:
#         for fn in [2, 4, 8, 16, 32, 64, 128]:
#             cost = test(bx, by, fn)
#             print(bx, by, fn, cost)
#             if cost < best:
#                 best = cost
...
#                 worst_mem = [bx, by, fn]
#
# print("for ({},{},{}) the best is [bx={},by={},fn={}] and time cost is {}".format(M, N, K, best_mem[0], best_mem[1], best_mem[2], best))
# print("for ({},{},{}) the worst is [bx={},by={},fn={}] and time cost is {}".format(M, N, K, worst_mem[0], worst_mem[1], worst_mem[2], worst))