...
def pytorch_cpu(batch_size, length, channel, kernel_size, output_channel, stride=1, padding=0, dilation=1, groups=1, number=100, dev=0):
    run_time = timeit.timeit(setup= 'import torch\n'
                                    ...
                                    'W = torch.rand([' + str(output_channel) + ', ' + str(channel//groups) + ', ' + str(kernel_size) + '], dtype=torch.float32)\n'
...
def pytorch_cuda(N, L, C, kernel_size, K, stride=1, padding=0, dilation=1, groups=1, number=100, dev=0):
    ...
    W = torch.rand([K, C//groups, kernel_size], dtype=torch.float32).cuda("cuda:" + str(dev))

    # warm-up
    torch.nn.functional.conv1d(A, W, stride=stride, padding=padding, dilation=dilation, groups=groups)
    ...
    for i in range(number):
        ...
        start.record()
        ans = torch.nn.functional.conv1d(A, W, stride=stride, padding=padding, dilation=dilation, groups=groups)
        end.record()
...
if __name__ == "__main__":
    ...
    for i, shape in enumerate(shapes):
        ...
        cost = baseline(batch, length, in_channel, k_len, out_channel, stride=stride, padding=padding, dilation=dilation, groups=groups, number=args.number, dev=args.device)
        print("Use %f(ms)" % cost)
    print("Done!")
