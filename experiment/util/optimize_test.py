...
def optimize(category, name, shapes, trans, slevel=4, rlevel=3, target="llvm", dev_id=0, timeout=4.0, trials=100, parallel=1,
             method="searching", use_model=False, logfile=sys.stdout):
    ret = dict()
    for i, shape in enumerate(shapes):
        print("Optimize {} shape {}".format(name, shape), flush=True)
...
def main(category, name, shapes, trans):
