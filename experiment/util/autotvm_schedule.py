...
def conv_transpose2d_cuda_schedule(N, C, H, W, K, Hk, stride, padding, dilation):
    ...
    cfg = autotvm.get_config()
    conv = topi.cuda.conv2d_transpose_nchw.conv2d_transpose_nchw_cuda(
        cfg, data, kernel, (stride, stride), (padding, padding), "float32")
    ops = [conv.op]
    # ops, [data, kernel, conv] = task.conv_transpose2d(
    #     N, C, H, W, K, Hk, stride, padding, dilation)
    s = tvm.create_schedule(ops)
...
def conv_transpose2d_evaluate(func, shape, target, dev):
    ...
    c_np = testing.conv2d_transpose_nchw_python(
        a_np, w_np, (stride, stride), (padding, padding))
